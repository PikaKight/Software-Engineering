...
let IHOST = "";
let IPORT = singleton.getPort();

while (IPORT == PORT){
  IPORT = singleton.getPort();
}

for (let i = 0; i < imgs.length; i++){
  imageKeysList[i] = `${imgs[i]}, ${singleton.getKeyID(imgs[i])}`;
}
...
if (process.argv.length > 2) {
  ...
  imageServerSocket.listen(IPORT, IHOST);
  console.log(`ImageDB server is started at timestamp ${singleton.getTimestamp()} and is listening on ${IHOST}:${IPORT}`)
  ...
  imageServerSocket.on("connection", (sock) => {
    imgHandler.handleClientJoining(sock);
  });

  imageServerSocket.on("data", (sock) => {
    imgHandler.handleClientJoining(sock);
  });
