...
function communicate(clientSocket, clientName, clientDHTtable) {
  ...
  clientSocket.on('data', (message) => {
    ...
    if (kadPacket.msgType == 1) {
      ...
      console.log(
        `Connected to ${senderPeerName}:${clientSocket.remotePort} at timestamp: ${singleton.getTimestamp()}\n`
      );
      ...
      console.log(
        `This peer address is ${clientSocket.localAddress}:${myReceivingPort} located at ${clientName} [${localPeerID}]\n`
      );
